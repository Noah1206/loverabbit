...
전환 스위치를 만들어 뒀습니다. `saju-facts.ts` 의
`BRANCH_YIN_YANG_MODE = "che"` 를 `"bongi"` 로 바꾸면 끝입니다.
**기본값은 `"che"` 로 뒀습니다** — 지금까지 발급된 리딩이 그 기준으로 나갔고,
80%를 바꾸는 결정을 제가 혼자 할 수 없기 때문입니다.
...
형충회합 중 **형만 구현돼 있지 않습니다.** 인사신 삼형, 축술미 삼형, 자묘 상형,
진진·오오·유유·해해 자형이 전부 빠져 있습니다. 관계 리딩에서 형은 갈등·구설·집착으로
보는 자리라, 이별·권태기·바람기 상품에서는 특히 아쉬운 공백입니다.

### 여자 명식의 상관운에 성별 분기가 없습니다

`LUCK-SIKSANG` 이 성별 구분 없이 '표현이 커진다'고만 봅니다.
여자 사주에서 관성은 남편이고 상관이 그 관성을 극하므로(상관견관), 여자 명식의
상관운은 관계를 흔드는 쪽으로 보는 시각이 있습니다. 관성/재성은 이미 성별로 갈라놨는데
식상만 안 갈라놨습니다.
...
제가 우선순위를 76에서 46으로 내린 것은 명리 교정이 아니라 **제품 판단**입니다 —
3분의 2가 공유하는 신호는 그 사람을 설명하지 못하므로 다른 규칙에 자리를 내주게 한 것입니다.
명리적으로는 되돌려도 틀리지 않습니다.
