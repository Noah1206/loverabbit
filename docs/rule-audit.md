...
> **이 문서는 2026-08-20 시점의 스냅샷이고, 지금 코드와 어긋나 있습니다.**
> 아래 표를 그대로 믿지 마세요.
>
> - 규칙 수: 문서 65건 / 현재 71건
> - `pyeongsaeng`(평생 연애운 신점)은 2026-08-21 에 내려간 상품입니다.
>   아래 `적용 상품` 칸 곳곳에 아직 남아 있지만, 코드에서는 빠졌습니다.
>
> 표를 부분적으로 고치지 않은 것은 일부러입니다. 이 문서는 그날 무엇을
> 검수했는지의 기록이고, 몇 군데만 손대면 최신인 척하는 문서가 됩니다.
> 다시 검수할 때 통째로 새로 뽑는 것이 맞습니다.
...
| **화개가 66% 발화** — 삼합의 고지(진술축미)가 넉 자 중 하나만 걸려도 잡혀, 그 사람을 구별하지 못함 | 우선순위를 76 → 46 으로 내렸다가 **76으로 되돌렸습니다.** 아래 4절 참조 — 계산이 정확했고, 낮출 근거가 없었습니다 |
...
| `SIN-HWAGAE` | 76 | 66.0% | shinsal=화개 | bimil gwontaegi bamgijil pyeongsaeng |
...
2026-08-20 추가. 형충회합 중 유일하게 빠져 있던 것입니다.
발화율은 무작위 명식 768건 기준이며, **부분 삼형 정책이 `on` 일 때**의 값입니다.
괄호 안은 `off` 일 때입니다.

| ID | 우선 | 발화율 | 조건 | 상품 |
|---|---|---|---|---|
| `XING-SPOUSE-PALACE` | 86 | 32.8% (14.8%) | 원국의 형이 일지에 걸림 | gyeolhon ibyeol gwontaegi jaehoe sokgunghap pyeongsaeng |
| `XING-ZIMAO` | 82 | 1.3% (1.3%) | 자묘 상형 | ibyeol gwontaegi sokgunghap bamgijil pyeongsaeng |
| `XING-YINSISHEN` | 80 | 16.7% (1.0%) | 인사신 삼형 | ibyeol gwontaegi jaehoe pyeongsaeng bamgijil |
| `XING-CHOUXUWEI` | 80 | 10.2% (0.5%) | 축술미 삼형 | gwontaegi ibyeol gyeolhon pyeongsaeng |
| `XING-LUCK-NOW` | 79 | 63.0% (43.8%) | 대운·세운의 글자가 낀 형 | ibyeol jaehoe gwontaegi hwanseung insun yeonae |
| `XING-SELF` | 74 | 28.9% (28.9%) | 진진·오오·유유·해해 자형 | gwontaegi sseom jjak bimil pyeongsaeng yeonae |
| `XING-LUCK-MONTH` | 70 | 60.7% (12.5%) | 월운의 글자가 낀 형 | jaehoe sseom yeonae hwanseung |

<details><summary>주장과 근거</summary>

**`XING-SPOUSE-PALACE`**
- 주장: 배우자 자리가 형에 걸려, 다른 관계에선 안 나오는 문제가 가까운 사이에서만 반복되는 구조
- 근거: 궁위 + 형 — 일지는 배우자궁. 그 자리의 형은 관계 안에서만 드러나는 마찰로 본다.
- 금지: 이혼한다 / 결혼 운이 없다 / 배우자가 문제다

**`XING-ZIMAO`**
- 주장: 가까워질수록 말이 거칠어지는 자리라, 감정보다 말투가 먼저 사이를 깎는 구조
- 근거: 형 — 자묘 상형(무례지형). 예의가 무너지는 방향으로 본다.
- 금지: 폭언한다 / 성격이 나쁘다 / 헤어진다

**`XING-YINSISHEN`**
- 주장: 들인 만큼이 그대로 돌아오지 않는 자리라, 애쓴 쪽이 먼저 지치기 쉬운 구조
- 근거: 형 — 인사신 삼형(무은지형). 은혜가 은혜로 돌아오지 않는 어긋남으로 본다.
- 금지: 배신당한다 / 이용당한다 / 손해를 본다

**`XING-CHOUXUWEI`**
- 주장: 서로 물러설 근거가 있어서 오히려 안 굽히는 자리라, 옳고 그름을 가리다 사이가 상하는 구조
- 근거: 형 — 축술미 삼형(지세지형). 각자 세력을 믿고 밀어붙이는 충돌로 본다.
- 금지: 싸움이 끊이지 않는다 / 결국 갈라선다

**`XING-LUCK-NOW`**
- 주장: 지금 지나는 흐름이 명식의 글자와 형을 이뤄, 평소엔 넘어가던 지점이 이 구간에만 크게 걸리는 때
- 근거: 형 + 운 — 대운·세운의 지지가 원국 지지와 형을 이룰 때
- 금지: 올해 헤어진다 / 이 시기에 반드시 일이 생긴다 / 지금 만나면 안 된다

**`XING-SELF`**
- 주장: 밖에서 온 문제보다 혼자 되짚는 시간이 관계를 더 흔드는 구조
- 근거: 형 — 자형(自刑). 같은 글자가 겹쳐 밖이 아니라 안에서 긁는 것으로 본다.
- 금지: 자학한다 / 우울증이 있다 / 정신적으로 문제가 있다

**`XING-LUCK-MONTH`**
- 주장: 이달만 유독 같은 대목에서 걸리는 흐름이라, 길게 볼 신호로 삼기엔 이른 때
- 근거: 형 + 월운 — 월운의 형은 가장 짧게 지나므로 구조로 읽지 않는다.
- 금지: 이번 달에 연락이 온다 / 이번 달에 끝난다

</details>

**검수 요청** — 형을 관계로 옮기는 방향(무은지형=어긋남, 지세지형=고집 충돌,
무례지형=말투, 자형=내향)이 타당한지, 그리고 부분 삼형을 실질로 보는 것이
맞는지 봐주십시오.

---

### 여자 상관운 (`LUCK-SANGGWAN-GYEONGWAN-F`) — 1건

2026-08-20 추가. 여자 상관운 정책이 실제로 말할 수 있는 **전부**입니다.

| ID | 우선 | 발화율 | 조건 | 상품 |
|---|---|---|---|---|
| `LUCK-SANGGWAN-GYEONGWAN-F` | 83 | 8.1% | 여자 + 상관운 + 명식에 관성 있음 + 정책 켜짐 | ibyeol gwontaegi gyeolhon jaehoe yeonae |

<details><summary>주장과 근거</summary>

- 주장: 지금 흐름이 하고 싶은 말을 밀어내는 쪽으로 서 있어, 참아 온 말이 관계의 규칙과 부딪히기 쉬운 때
- 근거: 십성 + 운 — 상관견관. 여자 사주에서 관성은 배우자성이고 상관이 그것을 극한다.
  명식에 관성이 실제로 있을 때만 성립하며, 결과가 아니라 마찰의 방향으로만 읽는다.
- 금지: 남편과 싸운다 / 이혼한다 / 관계가 끝난다 / 남자 복이 없다 / **말을 참아야 한다**

</details>

마지막 금지선이 핵심입니다. 상관견관을 "여자는 말을 참아라"로 옮기는 것은
명리 해석이 아니라 규범이고, 그 말은 하지 않습니다.

**검수 요청** — 상관견관을 관계 사안으로 읽는 것 자체가 타당한지,
그리고 이 문장이 그 원리를 과장 없이 옮긴 것인지 봐주십시오.
...
> **누가 했는가:** 개발자(Claude)가 고전 원칙과 대조한 것입니다.
> **자격 있는 명리학자의 검수가 아닙니다.** 아래 '정확한 것'은 제가 아는 한 맞다는 뜻이고,
> '결정이 필요한 것'은 유파가 갈려 제가 정할 수 없는 것입니다.

대조 범위: `reading-rules.ts` 57건, `saju-facts.ts` 십성·강약, `saju-shinsal.ts` 신살표

## 1. 대조해서 맞았던 것

### 오행 성정 (SELF-* 16건)

木仁·火표현·土信·金義·水智 배당이 고전과 일치합니다. 신강/신약 방향도 맞습니다 —
신약한 火가 스스로 불씨를 못 지킨다, 신강한 金이 설명보다 정리가 먼저 나간다 같은 서술이
각 오행의 본성에서 바르게 나왔습니다.

### 십성 정의 (TG-* 10건)

열 가지 모두 정/편의 음양 구분이 정확합니다.
...
규칙의 `근거` 도 맞습니다. 상관을 '정관을 극한다'고 쓴 것도 정확합니다 —
일간이 甲이면 상관은 丁(화), 정관은 辛(금), 火剋金 이므로 실제로 극합니다.

### 궁위와 성별 비대칭 (SPOUSE-*, LUCK-*)

일지를 배우자궁으로 보는 것, **남자는 재성이 처 · 여자는 관성이 부**로 갈리는 것이
규칙과 대운 규칙에서 일관됩니다. 남자의 관성운을 '직위·책임'으로, 여자의 재성운을
'활동·재물'로 따로 둔 것도 맞습니다. 이 비대칭이 규칙 계층에서 가장 잘 서 있는 부분입니다.

### 신살표 (SIN-* 6건)

| 신살 | 규칙 | 대조 |
|---|---|---|
| 도화 | 삼합 생지의 다음 글자 — 신자진→유, 해묘미→자, 인오술→묘, 사유축→오 | ✓ 표준 |
| 역마 | 삼합 생지의 충 — 신자진→인, 해묘미→사, 인오술→신, 사유축→해 | ✓ 표준 |
| 화개 | 삼합의 고지 — 진·미·술·축 | ✓ 표준 |
| 원진 | 자미·축오·인유·묘신·진해·사술 | ✓ 표준 |
| 양인 | 양간의 제왕 — 갑묘·병무오·경유·임자, 음간은 안 봄 | ✓ 다수설 |
| 홍염 | 갑을오·병인·정미·무기진·경술·신유·임자·계신 | ✓ 흔한 판본 (판본 갈림) |

### 형충회합 (REL-*, PAIR-*)

충·육합·삼합·천간합 정의가 정확하고, 천간합을 합화(合化)와 기반(羈絆)으로 나눠 본 것도 맞습니다.
일간합을 '서로 앞에서만 태도가 달라진다'로 옮긴 것은 기반의 관계적 해석으로 타당합니다.

---

## 2. 결정이 필요했던 것 — 지시로 정해졌습니다

> **아래 셋은 2026-08-20 운영자의 지시로 정해졌습니다.**
> **지시는 판정이 아닙니다.** 어느 쪽이 명리적으로 옳은지는 지금도 열려 있고,
> 그래서 셋 다 환경변수 하나로 되돌아갑니다. 전문가 검수의 초점은 여기입니다.
>
> | 항목 | 채택 | 되돌리기 |
> |---|---|---|
> | 지지 음양 | `main_hidden_stem` (본기) | `BRANCH_YIN_YANG_MODE=body` |
> | 부분 삼형 | 실질 성립으로 봄 | `XING_PARTIAL_POLICY=off` |
> | 여자 상관운 | 켬 | `FEMALE_SHANGGUAN_POLICY=off` |

### ★ 지지의 음양을 무엇으로 볼 것인가 (가장 큽니다)

지금 코드는 **체(體)의 음양**을 씁니다 (`jiIdx % 2` — 자인진오신술=양).
자평명리에서 지지의 십성을 논할 때는 **지장간 본기의 음양**을 쓰는 쪽이 다수입니다.

네 글자에서 갈립니다 — 체와 용이 어긋나는 자오사해입니다.
...
```
지지 자리 8,603곳 중 2,722곳(31.6%)에서 십성이 달라짐
명식 2,000개 중 1,599개(80.0%)가 최소 한 자리 영향
뒤집힘: 비견↔겁재 600 · 편인↔정인 543 · 식신↔상관 553 · 편관↔정관 513 · 편재↔정재 513
```

**정/편이 뒤집히면 해석이 반대가 됩니다.** 식신은 '표현으로 관계를 데운다',
상관은 '틀을 답답해하고 반박처럼 들린다' — 같은 사람이 정반대로 읽힙니다.

**2026-08-20, 본기 쪽으로 전환했습니다.** 운영자의 지시입니다 —
제가 근거를 찾아 정한 것이 아니고, 위 실측은 "얼마나 달라지는가"이지
"어느 쪽이 옳은가"가 아닙니다. 그 판정은 지금도 없습니다.

스위치는 환경변수 `BRANCH_YIN_YANG_MODE` 입니다. `body` 로 두면 이전 기준으로
돌아갑니다. 이미 발급된 리딩은 `SajuFacts.policy` 에 찍힌 표식으로 구분됩니다.
회귀 테스트는 **두 모드를 모두** 못박고 있어, 어느 쪽으로 움직여도 나머지가
조용히 무너지지 않습니다.

**검수 요청** — 이 표에서 갈리는 네 지지(자·사·오·해)에 대해, 십성을 매길 때
어느 음양을 쓰는 것이 맞는지 판정해 주십시오. 골든 케이스 100건으로 재는 방법은
`docs/audit/myeongri-regression-report.md` 에 있습니다.

### 홍염 판본

홍염은 자료마다 표가 갈립니다. 코드가 쓰는 판본을 위 표에 적어뒀으니,
다른 판본을 쓰신다면 `saju-shinsal.ts` 의 `HONGYEOM` 배열만 바꾸면 됩니다.

---

## 3. 빠졌던 것 — 채웠습니다

### ~~형(刑)이 없습니다~~ → 채움 (2026-08-20)

형충회합 중 형만 빠져 있었습니다. 인사신·축술미 삼형, 자묘 상형,
진진·오오·유유·해해 자형을 전부 넣고, **대운·세운·월운까지** 봅니다.
규칙 7건은 위 `XING-*` 절에 있습니다.

원국의 형(`xing`)과 운의 형(`xingLuck`)은 끝까지 갈라져 나갑니다 —
"늘 그렇게 걸리는 자리"와 "이 구간에만 겹치는 것"이 한 문장에 섞이지 않게 하기 위해서입니다.
**시기를 짚는 말은 오직 운 쪽에서만** 나옵니다.

### ~~여자 명식의 상관운에 성별 분기가 없습니다~~ → 채움 (2026-08-20)

`LUCK-SANGGWAN-GYEONGWAN-F` 하나로 채웠습니다. 조건은 세 겹입니다 —
상관운이 실제로 들어왔고, 명식에 부딪힐 관성이 실제로 있고, 성별이 명시됐을 때만입니다.
정책 스위치(`FEMALE_SHANGGUAN_POLICY`)로 통째로 끌 수 있습니다.

### 지지를 본기 오행만으로 읽습니다

지장간의 중기·여기를 보지 않습니다. 통근·투간 판단이 빠지므로 신강신약이 거칠어집니다.
간이 만세력의 일반적인 단순화이고 관계 리딩 수준에서는 통용되지만, 짚어둡니다.

---

## 4. 앞서 적은 것 정정

이 문서 위쪽에 **화개 66% 발화**를 두고 '계산이 과하게 잡는 것인지' 물었는데,
대조해보니 **계산은 정확합니다.** 화개는 삼합의 고지(진술축미)이고, 연지나 일지가
그 자체로 고지면 스스로 화개가 됩니다. 진술축미가 12지 중 4개이므로 자기 적중이 잦고,
연지·일지 두 기준을 함께 보니 더 잦아집니다. **명리적으로 정상입니다.**

제가 우선순위를 76에서 46으로 내린 것은 명리 교정이 아니라 **제품 판단**이었습니다 —
3분의 2가 공유하는 신호는 그 사람을 설명하지 못하므로 다른 규칙에 자리를 내주게 한 것입니다.

**그 강등은 되돌렸습니다. 지금은 76입니다.** 계산이 정확한 것을 계산 문제인 양
낮춘 것이 잘못이었고, 노출을 조절할 필요가 있다면 우선순위가 아니라 별도의 노출
축으로 해야 합니다(`docs/myeongri/rule-boundaries.md`).

같은 문제가 **`XING-LUCK-NOW`(63.0%)** 에도 있습니다. 이쪽은 처음부터 우선순위를
79로 잡아, 명식 고유의 형(80~86)보다 뒤에 서게 했습니다. 계산은 그대로 두고
자리만 뒤로 미룬 것입니다.
