...
export default function AdminPaymentsPage() {
  ...
  const review = async (order: PendingOrder, decision: "paid" | "cancelled") => {
    const action =
      decision === "paid"
        ? order.kind === "chat_credits" ? "입금을 승인하고 러빗을 지급" : "입금을 승인하고 풀 리딩을 열기"
        : "이 요청을 거절";
          ...
          {orders.map((order) => (
            <article className="card admin-order-card" key={order.id}>
              <div className="admin-order-title">
                ...
                <span className="badge">{order.kind === "chat_credits" ? "질문 러빗" : "리딩"} · 승인 대기</span>
              </div>
              <dl>
                <div><dt>주문코드</dt><dd>{order.depositorCode ?? "없음"}</dd></div>
                <div><dt>회원</dt><dd>{order.email ?? `회원 #${order.userId}`}</dd></div>
                <div><dt>상품</dt><dd>{order.kind === "chat_credits" ? `${String(order.metadata?.credits ?? "?")}러빗` : order.category ?? "사주 리딩"}</dd></div>
