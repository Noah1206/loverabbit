...
import { sumUsage, type ChatPromptCache, type ChatUsage } from "@/lib/ai";
...
export type Complete = (
  ...
  user: string,
  maxTokens: number,
  options?: { promptCache?: ChatPromptCache }
...
/**
 * 결제 전에 확정된 머리와 첫 절에서 뽑은 짧은 연결 상태.
 *
 * 같은 모델을 다시 부르는 것만으로는 앞 호출을 기억하지 못한다. 결제 후 조각들이
 * 무료 공개분의 결론을 뒤집거나 같은 말을 반복하지 않도록, 필요한 문체·결론만
 * 작게 접어 모든 후속 호출의 공통 입력으로 보낸다. 원문 전체를 다시 싣지 않는 것은
 * 이어쓰기 비용을 미리보기 길이만큼 계속 늘리지 않기 위해서다.
 */
...
/** 저장·후속 호출에 쓸 연결 상태를 결정론적으로 만든다. 모델을 추가로 부르지 않는다. */
export function continuityFromReport(report: StructuredReport): ReadingContinuityState {
  return {
    headline: compact(report.meta.headline, 180),
    summaryCards: report.summaryCards.slice(0, 4).map((card) => ({
      label: compact(card.label, 40),
      value: compact(card.value, 120),
    })),
    completedSections: report.sections.slice(0, 4).map((section) => ({
      title: compact(section.title, 120),
      verdict: compact(section.verdict, 220),
      summary: compact(section.summary, 280),
      styleAnchor: compact(section.paragraphs[0], 360),
    })),
    instruction: "앞에서 확정된 판단과 말투를 유지하고, 완료된 절의 내용을 반복하지 말고 다음 목차부터 이어 쓴다.",
  };
}
 ...
 * 첫 절은 실제 유료 본문의 시작으로 읽히고, 그 뒤는 제목만 목차에 남으므로
 * 결제 전에 만들 이유가 없다.
 *
 ...
 *
 * 저장된 리딩은 완료 절 수만큼 목차를 정확히 잘라 이어 쓰므로, 이후 이 값을 바꿔도
 * 옛 2절 미리보기의 다음 절을 건너뛰지 않는다.
 */
export function previewSections(): number {
  const raw = Number(process.env.READING_PREVIEW_SECTIONS);
  if (!Number.isInteger(raw) || raw < 1 || raw > 4) return 1;
  return raw;
}
...
export function previewBatchCount(outline: string[]): number {
  ...
  for (let i = 0; i < batches.length; i += 1) {
    ...
    if (items >= previewSections()) return i + 1;
  }
 ...
 */
function batchesOf(outline: string[], firstBatchSize?: number): Batch[] {
  ...
  /*
    ...
    통째로 만들었다. 그런데 결제 전에 보여 주는 절은 하나뿐이다. 셋을 만들어 둘을
    버린 셈이고, 그 하나는 **결제하지 않는 사람에게도** 매번 만들어졌다.
    광고 유입처럼 전환이 낮은 길에서는 그 낭비가 클릭 수만큼 곱해진다.

    첫 묶음만 떼어 내고 나머지는 예전처럼 고르게 나눈다. 결제 뒤에는 저장된 완료
    절 수로 목차를 먼저 자른 뒤 남은 항목만 새로 묶는다.
  */
  if (firstBatchSize !== undefined) push(Math.min(firstBatchSize, outline.length), 0);
...
export function chaptersOf(outline: string[]): Batch[] {
  return batchesOf(outline, previewSections());
}

/** 결제 후 남은 목차는 무료 공개 경계 없이 보통 묶음 크기로 고르게 자른다. */
function continuationChaptersOf(outline: string[]): Batch[] {
  return batchesOf(outline);
}
...
/**
 * 공통 입력을 조각별 지시와 분리해 정확한 캐시 프리픽스로 만든다.
 *
 * key에는 생년 정보 원문이나 그 해시를 직접 싣지 않고 16개 라우팅 버킷만 쓴다.
 * 같은 리딩은 언제나 같은 버킷으로 가고, 버킷을 나눠 key 하나에 트래픽이 몰리는
 * 것도 피한다. 실제 일치 여부는 공급사가 key와 정확한 프리픽스를 함께 검사한다.
 */
function readingPromptCache(
  inputJson: string,
  ...
  const bucket = digest.readUInt16BE(0) % 16;
  return {
    key: `paid-reading:v3:${bucket}`,
    prefix,
  };
}
...
export interface ComposeOptions {
   ...
   */
  doneSections?: number;
  /** 결제 전에 확정된 머리·첫 절의 결론과 문체. 후속 조각만 받는다. */
  continuity?: ReadingContinuityState;
}
...
export async function composeReport(
  ...
  const doneSections = Math.min(
    Math.max(0, options.doneSections ?? 0),
    input.outline.length
  );
  const resuming = doneSections > 0;
  const promptCache = readingPromptCache(inputJson, resuming ? options.continuity : undefined);
  // 이미 만든 절의 수만큼 목차를 정확히 자른 뒤 새로 묶는다. 예전 미리보기가 2절이고
  // 지금 기본값이 1절이어도 옛 묶음 경계 때문에 3·4절을 건너뛰지 않는다.
  const all = resuming
    ? continuationChaptersOf(input.outline.slice(doneSections))
    : chaptersOf(input.outline);
  const chapters = all
    .slice(0, options.batchLimit === undefined ? undefined : options.batchLimit);
  // 캐시 쓰기도 과금되는 GPT-5.6에서 한 번만 부를 프리픽스를 쓰는 것은 손해다.
  // 실제로 재사용할 본문 조각이 둘 이상일 때만 명시적 캐시를 켠다.
  const bodyPromptCacheEnabled = chapters.length > 1;
  ...
  const run = async (
    ...
    try {
      const result = await complete(systemPromptFor(branch), prompt, budget, {
        // prefix는 항상 전달해야 모델이 입력 JSON을 본다. enabled만 갈라 유료 캐시
        // 쓰기를 실제 재사용 가능한 본문 호출에서만 만든다.
        promptCache: { ...promptCache, enabled: branch === "body" && bodyPromptCacheEnabled },
      });
    ...
    const retried = await Promise.all(
      needsRetry.map(({ chapter, index }) => {
        ...
        return run(
          `${chapter.label} 재시도`,
          "body",
          chapterPrompt(target, input.outline, index === 0 ? season : ""),
...
export async function rewriteFlagged(
  ...
  const inputJson = buildReadingInput(input);
  const promptCache = readingPromptCache(inputJson);
  const usages: (ChatUsage | null)[] = [];
  let requestCount = 0;
    ...
    flagged.map(async ({ title, notes }) => {
      ...
      try {
        const result = await complete(
          systemPromptFor("body"),
          chapterPrompt({ label: "다시", items: [title] }, input.outline, "", notes),
          chapterBudget(1),
          { promptCache: { ...promptCache, enabled: true } }
        );
        if (!result) return null;
        usages.push(result.usage ?? null);
