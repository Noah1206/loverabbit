...
const STRENGTH_GUIDELINE: Record<"신강" | "중화" | "신약", Guideline> = {
  신강: {
    title: "멈출 장치를 둬",
    body: "너는 시작이 빠른 쪽이다. 대신 그만두는 게 늦다. 벌일 때 끝낼 조건부터 적는 걸 추천해.",
    basis: "강약",
  },
  중화: {
    title: "기준 하나만 정해",
    body: "너는 어느 쪽으로도 잘 기운다. 그만큼 기준이 흐려지기 쉽다. 결정 전에 먼저 볼 것 하나만 정하자.",
    basis: "강약",
  },
  신약: {
    title: "한 번에 하나만",
    body: "너는 살피는 쪽이 세다. 여러 개를 안으면 빨리 닳는다. 벌인 걸 줄이는 게 새로 시작하는 것보다 이득이다.",
    basis: "강약",
  },
};
...
const EXCESS_GUIDELINE: Record<Ohaeng, Guideline> = {
  목: { title: "벌인 걸 세어봐", body: "너는 뻗는 힘이 세다. 계획이 자꾸 는다. 새로 시작하기 전에 열려 있는 걸 먼저 세자.", basis: "오행 목" },
  화: { title: "한 박자만 늦춰", body: "너는 반응이 빠르다. 그 속도가 무기다. 중요한 자리에서만 한 박자 늦추는 걸 추천해.", basis: "오행 화" },
  토: { title: "하나는 놓아봐", body: "너는 버티는 힘이 세다. 안 맞는 것도 오래 쥔다. 붙잡은 것 하나를 의심해보자.", basis: "오행 토" },
  금: { title: "덜 다듬고 내", body: "너는 기준이 분명하다. 그래서 끝없이 고친다. 완성 전에 한 번 보여주는 게 빠르다.", basis: "오행 금" },
  수: { title: "생각을 꺼내", body: "너는 안에서 오래 굴린다. 머릿속에선 정리가 안 된다. 적거나 말하면 바로 가벼워진다.", basis: "오행 수" },
};

/** 어느 오행이 없을 때의 지침 — 부족이 아니라 "손이 덜 가는 자리"로 쓴다 */
const ABSENT_GUIDELINE: Record<Ohaeng, Guideline> = {
  목: { title: "시작엔 마중물", body: "먼저 뻗는 건 네 결이 아니다. 의지 말고 시간과 자리를 먼저 정하자. 그게 시작을 쉽게 만든다.", basis: "오행 목 없음" },
  화: { title: "떠오르면 바로 보내", body: "드러내는 건 네 결이 아니다. 마음이 있어도 전달이 늦는다. 떠올랐을 때 한 줄 바로 보내는 걸 추천해.", basis: "오행 화 없음" },
  토: { title: "돌아올 자리 하나", body: "버티는 건 네 결이 아니다. 흔들릴 때 기댈 데가 필요하다. 사람이든 습관이든 돌아올 자리 하나를 정하자.", basis: "오행 토 없음" },
  금: { title: "마감은 밖에 맡겨", body: "끊는 건 네 결이 아니다. 기준을 네 판단에 두면 밀린다. 달력과 약속에 맡기자.", basis: "오행 금 없음" },
  수: { title: "큰 결정엔 하루", body: "오래 재는 건 네 결이 아니다. 판단이 빠른 만큼 되돌아봄이 적다. 큰 결정엔 하루를 얹자.", basis: "오행 수 없음" },
};
...
const TENGOD_GUIDELINE: Record<string, Guideline> = {
  비견: { title: "하나는 맡겨", body: "너는 혼자 끄는 결이다. 맡기는 게 어색하다. 하나만 넘겨보자 — 전체가 빨라진다.", basis: "십성 비견" },
  겁재: { title: "몫부터 말해", body: "너는 힘을 나누는 자리가 잦다. 나중에 갈리기 쉽다. 시작할 때 각자 몫을 못박는 걸 추천해.", basis: "십성 겁재" },
  식신: { title: "갈아타지 마", body: "너는 오래 하는 일에 세다. 성과가 늦게 보일 뿐이다. 중간에 갈아타지 않는 게 네 무기다.", basis: "십성 식신" },
  상관: { title: "상대 보고 세기 조절", body: "네 말은 빠르고 날카롭다. 무기가 되는 자리와 상처가 되는 자리가 갈린다. 상대를 보고 세기를 정하자.", basis: "십성 상관" },
  편재: { title: "보는 건 넓게, 손은 하나", body: "너는 여러 갈래를 동시에 본다. 손까지 여러 개면 흩어진다. 손대는 건 하나로 줄이자.", basis: "십성 편재" },
  정재: { title: "쌓이는 걸 보이게", body: "너는 차곡차곡 모으는 결이다. 진행이 안 보이면 힘이 빠진다. 숫자든 기록이든 보이는 데 두자.", basis: "십성 정재" },
  편관: { title: "마감은 남과 잡아", body: "너는 압박이 걸려야 움직인다. 혼자 만든 마감은 잘 안 듣는다. 남과 약속한 마감을 추천해.", basis: "십성 편관" },
  정관: { title: "틀부터 만들어", body: "너는 틀 안에서 안정적이다. 틀 없는 일이 힘들 뿐이다. 규칙을 먼저 만들고 시작하자.", basis: "십성 정관" },
  편인: { title: "네 방식으로 다시 정리", body: "너는 남과 다른 길로 이해한다. 표준 설명이 안 맞는 게 정상이다. 네 방식으로 정리하는 시간을 아끼지 마.", basis: "십성 편인" },
  정인: { title: "70%에서 내보내", body: "너는 채우고 나서 움직이는 결이다. 완벽을 기다리면 시기를 놓친다. 70%에서 한 번 내보내자.", basis: "십성 정인" },
};
