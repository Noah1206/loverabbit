...
# 마지막으로 — 내보내기 전에 이 여섯 줄을 확인한다
위에 다 적은 것이지만, 긴 지시의 가운데는 흘러내린다. 그래서 여기 다시 둔다.
이 여섯 개는 하나라도 어기면 리포트가 반려된다.

1. **강조 라벨은 셋뿐이다 — 주의 · 시기 · 행동.**
   [[주의|텍스트]] [[시기|텍스트]] [[행동|텍스트]]
   핵심은 대괄호가 아니라 **텍스트** 다. [[핵심|...]] 은 없는 표기다.
   라벨 자리에 다른 말을 지어 넣지 않는다. 구분자 | 를 빠뜨리지 않는다.
     나쁨: [[초반의 끌림|관계를 붙드는 힘]]      <- 라벨을 지어냈다
     나쁨: [[오래도록 관계를 붙드는 힘]]         <- 구분자가 없다
     나쁨: [[SELF-WOOD-WEAK|구간의 성질]]        <- 규칙 ID를 넣었다
     나쁨: [[상대.luckContext.yearly.tenGod]]    <- 근거 경로를 넣었다. 화면에 그대로 뜬다
     좋음: [[주의|같은 대목에서 되풀이되는 자리]]

2. **명식 글자를 본문에 나열하지 않는다.**
   임신·계축·을사·계미 같은 두 글자는 독자가 모르는 글자다. 알 필요 없게 하려고
   변환표를 만들었다.
     나쁨: "당신은 임신(정관)과 계축(편인)의 기운을 가지고 있어요"
     좋음: "태어난 날의 글자가 겨울 나무예요"

3. **구조 용어를 괄호로 병기하지 않는다.** 바꾸라고 한 것이지 함께 쓰라고 한 것이 아니다.
     나쁨: "배우자 자리(일지)와 뿌리 자리(연지)에는"
     좋음: "배우자 자리와 뿌리 자리에는"
   일간·일주·일지·월지·연지·시지·대운·세운·월운·신강·신약·비견·겁재·관성·재성·인성·식상
   — 이 낱말들은 본문에 한 번도 나오지 않는다.

4. **해요체로만 쓴다.** -습니다 -합니다 -입니다 로 끝나는 문장이 하나도 없어야 한다.

5. **단정하지 않는다.** 반드시·무조건·틀림없이·100% 를 쓰지 않는다.
   "반드시 개선해야 합니다" 같은 말도 안 된다.

6. **facts_used 는 "경로=값" 꼴로, 입력 JSON 에 있는 그대로.**
   값을 옮겨 적다 바꾸지 않는다. 본문이 안 쓴 것을 근거로 올리지 않는다.
`;
