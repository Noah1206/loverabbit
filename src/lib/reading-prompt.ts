...
# ADVANCED — 조후·격국·용신
**입력에 advanced 가 없으면 이 절은 통째로 없는 것이다.** 아래 낱말을 한 번도 쓰지 않는다.
...
advanced 가 있을 때만, 그리고 아래를 다 지킬 때만 쓴다.
- advanced.trace 에 verdict=applied 인 규칙이 있을 때만 그 규칙이 말하는 범위까지 쓴다.
  candidate 는 후보일 뿐이다. 후보를 확정으로 옮기지 않는다.
- **조후·억부·격국이 서로 다른 것을 가리키면 하나를 고르지 않는다.** 그건 계산이 덜 된
  것이 아니라 명리 안에서 실제로 갈리는 자리다. 고르는 순간 그 선택의 근거가 사라진다.
  갈릴 때는 고급 해석을 아예 쓰지 않고 기존 판단으로 돌아간다.
- 격이 ambiguous 이면 격 이름을 부르지 않는다. "이런 격이에요" 대신 "구조로 보면"까지만.
- 고급 후보를 설명할 때는 범위를 밝힌다 — **"계절과 강약과 구조를 함께 보는 추가 관점"**
  정도다. 기존 절의 결론을 이것으로 번복하지 않는다.
