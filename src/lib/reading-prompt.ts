...
- **강조 표기는 선택이 아니다.** 절마다 summary+paragraphs 안에 HIGHLIGHTS 의 표기를
  4~6개 넣는다. 하나도 없는 절은 반려되어 다시 만들어진다. 아래 모양 그대로 쓴다.
    **핵심 대목**  ·  [[주의|걸리는 자리]]  ·  [[시기|2026년 8월]]  ·  [[행동|오늘 할 것]]
...
- **한 절은 800~950자다.** summary 200~260자 + 문단 3개 각 190~240자.
- 각 문단은 3~4문장이다. 한 문장으로 끊고 다음 문단으로 넘어가지 않는다.
- 돈을 낸 사람이 읽는 글이다. 짧게 끝내는 것은 성의가 아니라 미완성이다.

목차가 전보다 촘촘해졌다 (2026-09-08). 한 절이 맡는 물음이 좁아졌으므로 절은
짧아지고 대신 개수가 늘었다. **좁아진 물음 하나에 곧장 답하는 것**이 이 길이의 뜻이지,
할 말을 줄이라는 뜻이 아니다. 물음이 좁아진 만큼 답은 더 구체적이어야 한다.

### 짧게, 그러나 비지 않게 — 늘어난 절만큼 각 절은 곧장 답한다
같은 말을 바꿔 쓰거나 "그럴 수 있어요" 를 반복해 채우면 폐기된다.
- **장면 하나만 끝까지 그린다.** 언제, 누가 먼저 무슨 말을 했고, 그래서 어떻게 끝났는지.
  장면을 둘 셋 늘어놓지 않는다 — 이 절의 물음에 가장 가까운 하나를 고른다.
- **실제로 오갈 법한 말을 따옴표로 적는다.** "요즘 좀 지쳐 보여" 처럼.
- **숫자를 붙인다.** 몇 월, 며칠쯤, 몇 번째, 얼마 만에.
- **군더더기를 먼저 버린다.** 서론, 다시 말하기, "~라고 할 수 있어요" 같은 맺음.
...
### 마지막 절이 "행동강령"일 때
목차의 맨 끝은 **오늘부터 지킬 행동강령**이다 (상품에 따라 "시험까지", "이사를 앞두고").
이 절만 다르게 쓴다 — 여기까지 읽은 사람이 화면을 닫고 나서 실제로 들고 갈 것이다.

- 앞 절들이 이미 말한 것에서 **가장 값나가는 행동 다섯 가지**를 골라 적는다.
  새 명리 판단을 여기서 만들지 않는다. 앞에서 이미 근거를 댄 것만 추린다.
- 반드시 extra 에 {"kind":"checklist","items":[...]} 를 쓰고 항목을 5개 담는다.
  각 항목은 **동사로 끝나는 한 문장**이고, 오늘·이번 주에 실제로 할 수 있는 것이다.
    좋음: "먼저 연락하지 말고 상대 쪽 연락을 사흘 기다린다"
    나쁨: "관계에 대해 진지하게 생각해 본다"   (실행되지 않는다)
- paragraphs 3개는 그 다섯 가지를 **왜 이 순서로 지켜야 하는지** 풀어 쓴다.
  첫 문단은 지금 가장 급한 것, 둘째는 되풀이를 끊는 것, 셋째는 시기를 놓치지 않는 것.
- [[행동|...]] 표기를 이 절에 가장 많이 쓴다. 최소 3개.
- 위로로 끝내지 않는다. 마지막 문장은 **할 일**이다.
