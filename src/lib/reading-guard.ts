...
export const OUT_OF_SCOPE: [RegExp, string][] = [
  ...
  /*
    ...
    속궁합 목차 4장 01 이 이 세 이름으로 절을 판다. 사람들이 아는 이름으로
    절을 부르는 것은 되지만, **본문이 셋 중 하나를 찍는 것은 안 된다.**

    규칙 층이 내주는 것은 강약의 기울기(억부)뿐이다 — 힘이 어느 쪽으로 기우는가.
    낮과 밤의 대비는 거기서 나오지 않는다. 그런데 목차에 그 이름이 있으면
    모델은 답을 요구받았다고 읽고 셋 중 하나를 고르게 된다. 그것이 바로
    규칙 없이 만들어진 판정이다.

    그래서 본문은 뒤집힘의 방향까지만 쓰고, 이름을 찍으면 여기서 막힌다.
    규칙의 forbidden 에도 같은 말이 적혀 있지만 그쪽은 완전일치라
    ("낮이밤져다") 활용형을 못 잡는다.
  */
  [
    /낮[이져]밤[이져](\s*(스타일|타입|쪽|형))?\s*(이다|입니다|이에요|예요|이야|다|네요|군요|으?로\s*(보|읽|판단)|에\s*가깝|라고\s*볼)/,
    "낮이밤져 판정",
  ],
