...
import type { Gender } from "@/lib/saju-facts";

export type FixtureReviewState = "pending_expert_review" | "reviewed";
 ...
 * 검토된 기대값.
 *
 * **비어 있는 것이 지금의 정상 상태다.** 명리 전문가가 각 명식의 격국·용신 판정을
 * 확인하고 approvedPolicyAssertions 를 채워야 policy_enabled 로 갈 수 있다.
 * 그때까지 아래 함수는 빈 배열을 돌려주고, 회귀 테스트는 계산 경로만 지킨다.
 */
...
export function fixtureReviewSummary() {
  ...
  return {
    ...
    /** 하나라도 검토 전이면 policy_enabled 로 갈 수 없다 */
    gatesPolicyEnabled: reviewed < total,
  };
}

export const FIXTURE_SUITE_VERSION = "advanced-fixtures-v1-pending";
