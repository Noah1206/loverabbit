...
// 힉스필드 원본(cards-raw/*.mp4)을 웹에 올릴 크기로 줄이고,
// 어떤 카드에 클립이 있는지 목록(src/lib/card-motion.ts)을 다시 쓴다.
//
//   node scripts/build-card-motion.mjs
//   node scripts/build-card-motion.mjs --only sokgunghap
//
// 하는 일 세 가지:
...
const rawDir = path.join(root, "cards-raw");
...
const body = [
  "// 이 파일은 scripts/build-card-motion.mjs 가 생성한다. 손으로 고치지 마라.",
  "//",
