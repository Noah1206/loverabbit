...
| --- | --- | --- | --- | --- |
| 4주·대운·기본 strength | 임신 계축 을사 계미 / 경술(26~35세) / 신약 36 | 같음 | 같음 | not_run |
| 계절/한난조습 | 없음 | 축월 cold/wet | 축월 cold/wet | not_run |
| 격국 후보 | 없음 | ambiguous (편인격/medium, 편재격/medium, 편관격/low) | ambiguous (편인격/medium, 편재격/medium, 편관격/low) | not_run |
...
| 충돌 상태 | 계산 안 됨 | CONFLICT-EOKBU-JOHU-수화 · content_suppressed | CONFLICT-EOKBU-JOHU-수화 · content_suppressed | not_run |
| 사용자 리포트 결론 변경 | — | 안 바뀜 | 안 바뀜 | not_run |
| blocking/major/advisory | — | blocking 0 · major 1 · advisory 0 | 같음 | not_run |

`policy_enabled` 이 `not_run` 인 것이 정상이다 — 고정 명식 32건 중 32건이 아직 전문가 검토 전이고(advanced-fixtures-v1-pending), 그것이 이 모드의 문지기다.
...
판정 **ambiguous** · 상태 calculated_only · 대표 없음
...
| 후보 | 확신 | 근거 |
| --- | --- | --- |
| 편인격 | medium | 월지 축의 지장간 계(여기)가 일간 을에게 편인 / 계이 월간,시간에 투간했다 / 월지가 미충(시지)으로 흔들린다 — 격이 온전하지 않을 수 있다 |
| 편재격 | medium | 월지 축의 지장간 기(본기)가 일간 을에게 편재 / 기은 천간에 드러나지 않았다 / 월지가 미충(시지)으로 흔들린다 — 격이 온전하지 않을 수 있다 |
...
- **CONFLICT-EOKBU-JOHU-수화** (major / content_suppressed)
  - 억부는 수, 조후는 화를 가리킨다
  - 억부와 조후의 우선순위를 정한 승인 정책이 없다. 기존 해석을 그대로 두고 고급 결론을 내지 않는다. (축월은 한난이 극에 있는 자리라 CR-JOHU-FIRST-EXTREME-SEASON 이 겨냥하는 경우지만, 아직 draft 다.)

### 사용자에게 안 나가는 이유

- ADVANCED_MYEONGRI_MODE=evidence_only — 계산만 하고 사용자 글은 바꾸지 않는다
- 조후 후보 3개가 전부 승인 전이다 (johu-candidates-v1-draft)
- 격국이 ambiguous·calculated_only 라 단일 격으로 서술할 수 없다
- 축이 갈리는데 우선순위 정책이 승인되지 않았다 (CONFLICT-EOKBU-JOHU-수화)

프롬프트 입력에 실린 advanced: `null`

## 1. 지금 사용자 리포트에 쓸 수 있는 approved 규칙

**없다.** 조후·격국·용신 어느 축에서도 승인된 규칙이 0건이다.
...
| ADV-GYEOK-INNER-V1 | ambiguous | 후보 편인격, 편재격, 편관격 가 같은 무게로 서서 하나로 정하지 않는다 |
...
| ADV-YONGSIN-EOKBU-V1 | candidate | 억부 · 수 · 일간 목이 약하다는 판정(신약 26 제안)에서 나오는 후보 |
...
| CONFLICT-UNRESOLVED | blocked | 억부와 조후의 우선순위를 정한 승인 정책이 없다. 기존 해석을 그대로 두고 고급 결론을 내지 않는다. (축월은 한난이 극에 있는 자리라 CR-JOHU-FIRST-EXTREME-SEASON 이 겨냥하는 경우지만, 아직 draft 다.) |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| MZ-01-IN | 인월(초봄) — 계절 전환 직후 | 무진 갑인 을사 신사 | 인월 cool/balanced | 중화 | determined | insufficient_evidence | 0 |
...
| MZ-06-MI | 미월(조토) — 덥고 마른 자리 | 계해 기미 신해 병신 | 미월 hot/dry | 신강 | determined | unanimous | 0 |
...
| MZ-09-SUL | 술월(조토) — 서늘하고 마른 자리 | 신사 무술 무오 정사 | 술월 cool/dry | 신강 | ambiguous | insufficient_evidence | 0 |
| MZ-10-HAE | 해월(초겨울) | 을묘 정해 경오 계미 | 해월 cold/wet | 신약 | determined | conflict | 1 |
| MZ-11-JA | 자월(한겨울) — 수가 극에 있는 자리 | 기묘 병자 병오 기축 | 자월 cold/wet | 신약 | ambiguous | conflict | 1 |
| MZ-12-CHUK | 축월(겨울 끝) — 기준 명식과 같은 월지 | 임신 계축 을사 계미 | 축월 cold/wet | 신약 | ambiguous | conflict | 1 |
| TB-01-BEFORE-IPCHUN | 입춘 하루 전 — 연주가 갈리는 자리 | 을해 기축 경오 임오 | 축월 cold/wet (경계) | 중화 | determined | insufficient_evidence | 0 |
| TB-02-AFTER-IPCHUN | 입춘 하루 뒤 — 위와 짝 | 병자 경인 임신 병오 | 인월 cool/balanced (경계) | 중화 | ambiguous | insufficient_evidence | 0 |
| TB-03-TERM-EDGE | 절입 당일 — 경계 표시가 서야 한다 | 정묘 정미 기축 무진 | 미월 hot/dry (경계) | 신강 | ambiguous | conflict | 1 |
| TB-04-TERM-DEEP | 절입 한복판 — 경계 표시가 없어야 한다 | 정묘 무신 갑진 무진 | 신월 balanced/balanced | 신약 | ambiguous | insufficient_evidence | 0 |
| HR-01-EARLY-JA | 야자시 — 자시 앞쪽 | 임신 을사 병술 기해 | 사월 warm/balanced | 중화 | ambiguous | insufficient_evidence | 0 |
| HR-02-LATE-JA | 조자시 — 자시 뒤쪽, 날짜가 갈린다 | 임신 을사 병술 무자 | 사월 warm/balanced | 중화 | ambiguous | insufficient_evidence | 0 |
| HR-03-UNKNOWN | 시각 미상 — 시주가 서지 않는다 | 갑자 경오 경진 미상 | 오월 hot/dry | 신약 | ambiguous | conflict | 1 |
| ST-01-STRONG | 일간이 뿌리를 많이 둔 명식 | 경신 무인 정축 임인 | 인월 cool/balanced (경계) | 중화 | ambiguous | insufficient_evidence | 0 |
| ST-02-WEAK | 일간이 계절을 거스르고 설기가 많은 명식 | 을해 임오 을미 임오 | 오월 hot/dry | 중화 | ambiguous | insufficient_evidence | 0 |
| ST-03-EVEN | 중화에 가까운 명식 | 기사 무진 임인 무신 | 진월 balanced/wet | 신약 | determined | insufficient_evidence | 0 |
| GK-01-EXPOSED-MAIN | 월지 본기가 투간 — 격이 뚜렷할 후보 | 정사 경술 기해 정묘 | 술월 cool/dry (경계) | 신강 | ambiguous | insufficient_evidence | 0 |
| GK-02-NO-EXPOSURE | 월지 지장간이 하나도 투간하지 않음 | 계미 계해 경술 정해 | 해월 cold/wet | 중화 | determined | insufficient_evidence | 0 |
| GK-03-MONTH-CLASHED | 월지가 충을 맞음 — 격이 흔들리는 자리 | 임술 기유 경자 무인 | 유월 cool/balanced | 신강 | unsupported | insufficient_evidence | 0 |
| GK-04-MONTH-COMBINED | 월지가 육합에 묶임 | 무인 계해 무인 신유 | 해월 cold/wet | 신약 | ambiguous | unanimous | 0 |
...
| CF-02-SUMMER-WEAK | 여름생 신약 — 위의 반대쪽 | 을축 계미 임자 정미 | 미월 hot/dry | 신약 | ambiguous | conflict | 1 |
| CF-03-WINTER-STRONG | 겨울생 신강 — 조후와 억부가 같은 쪽을 볼 후보 | 무오 갑자 갑자 갑자 | 자월 cold/wet | 신강 | ambiguous | unanimous | 0 |
| CF-04-SUMMER-STRONG | 여름생 신강 | 갑신 경오 병진 계사 | 오월 hot/dry (경계) | 신강 | ambiguous | conflict | 1 |
| PT-01-PARTNER | 기준 명식의 상대 — 궁합 경로 회귀 | 신미 을미 기묘 갑술 | 미월 hot/dry (경계) | 중화 | determined | insufficient_evidence | 0 |

격국 판정: determined 14 / ambiguous 16 / unsupported 2

축 합의: insufficient_evidence 20 · conflict 9 · unanimous 3
...
| 2 | 격국 V1 내격 판정 범위 | 월지·지장간·투간 근거가 명확한 범위부터 시작할 수 있다 | 대기 |
| 3 | 억부 강약 정책 | P2의 가중치·통근·설기·인성과다 기준을 명시적으로 정해야 한다 | 대기 |
| 4 | 조후용신 표 (일간 × 월지 120칸) | 판본·번역·주석과 각 행의 근거를 확보한 뒤에만 승인할 수 있다 | 대기 |
| 5 | 격국·조후·억부 충돌 우선순위 | 단일 용신 결론을 사용자에게 보여 주기 직전의 마지막 정책이다 | 대기 |
